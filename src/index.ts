...
console.log(__dirname + '/cert/key.pem' + "l16");
...
const samlStrategy = new Strategy({
  ...
  ((profile, done) => done(null, profile as Record<string, unknown>)) as VerifyWithoutRequest,
  ((profile, done) => done(null, profile as Record<string, unknown>)) as VerifyWithoutRequest
);
...
app.get('/login/fail', (req: Request, res: Response) => res.status(401).send('Login failed'));
...
function RequestWithUser(err: Error | null, url?: string | null | undefined): void {
